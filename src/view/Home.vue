<template><div class="app-container"><div class="viewport"><div class="stage" :style="stageStyle"><!-- 页面背景图 --><img :src="bgImage" alt="背景" class="page-bg" /><!-- 顶部标题栏 --><header class="app-header"><img :src="headerImage" alt="顶部标题栏" class="header-image" /></header><!-- 左侧面板 --><LeftPanel /><!-- 右侧面板 --><RightPanel /><!-- 主内容区 --><main class="app-main"><CesiumViewer /></main><!-- AI 聊天输入框 --><AIChatInput /><!-- 底部导航组件 --><BottomNavigation /></div></div></div></template><script setup>import { ref, computed, onMounted, onBeforeUnmount } from 'vue'
...
}))</script><style scoped>.app-container {
  height: 100vh;
  width: 100vw;
  overflow: hidden;
}
